...
const {Topic} = require('../src/core/topic')
const {Marker} = require('../src/core/marker')
...
function isValidChild(childString, parentString) {
    ...
    allowedEmojiMap.set("priority-2", ["priority-1"]);
    allowedEmojiMap.set("priority-1", ["priority-1"]);
    allowedEmojiMap.set("priority-3", ["priority-1"]);
    allowedEmojiMap.set("priority-4", ["priority-2"]);
    allowedEmojiMap.set("priority-5", ["priority-2"]);
    allowedEmojiMap.set("priority-6", ["priority-3"]);
    ...
    if (listOfValidParents != undefined) {
        ...
        let filteredEmojis = listOfValidParents.filter((emoji) => {
            return emoji === parentString
        });
